...
const listPet = query(ref(database, 'account/'+userID+'/listPet'), limitToLast(100));
...
onValue(listPet,async (snapshot) => {
  let detailPetHTML =''
  const contentLeft = document.querySelector('#info-detail-pet .content .content-left')
  await snapshot.forEach((childSnapshot) => {
    var childData = childSnapshot.val();
    var childKey = childSnapshot.key;
    if(childKey == idPet)
    {
      let content = `<div class="avatar_box">
                            <div class="avatar">
...
const infoPetRef = query(ref(database, 'account/'+userID+'/listPet/' + idPet), limitToLast(100));

onValue(infoPetRef,async (snapshot) => {
    let dataWeightFood = [],dataWeightPet = [],rightHTML ='';
    const contentRight = document.querySelector('#info-detail-pet .content .content-right')
    await snapshot.forEach((childSnapshot) => {
        var childData = childSnapshot.val();
        var childKey = childSnapshot.key;
        ...
        {   
            ...
            for(let item in newObj)
            {
                dataWeightPet.push({y: item, weightPet: newObj[item][newObj[item].length - 1]});
            }
        }
    });
    if(dataWeightFood.length >10){
        dataWeightFood = dataWeightFood.slice(dataWeightFood.length-10,dataWeightFood.length);
    }
    if(dataWeightPet.length >10){
        dataWeightPet = dataWeightPet.slice(dataWeightPet.length-10,dataWeightPet.length);
    }
